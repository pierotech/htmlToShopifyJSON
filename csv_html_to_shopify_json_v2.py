...
BLOCK_TAGS = {
    ...
    'nav', 'pre', 'blockquote', 'figure', 'figcaption', 'table', 'thead',
    'tbody', 'tfoot', 'tr', 'td', 'th', 'form', 'fieldset', 'legend',
    'address', 'details', 'summary', 'hr', 'br', 'canvas', 'video', 'audio'
}

# Tags that should be treated as inline formatting
INLINE_FORMATTING_TAGS = {'strong', 'b', 'em', 'i', 'a'}

# Tags that should be ignored/skipped entirely (no content extracted)
SKIP_TAGS = {
    'script', 'style', 'noscript', 'iframe', 'embed', 'object', 'applet',
    'meta', 'link', 'base', 'title', 'head', 'comment'
}
...
def html_to_json(html: str) -> str:
        ...
        for child in element.children:
            if isinstance(child, NavigableString):
                ...
                if child.name in SKIP_TAGS:
                    ...
                    # For any other tag not in inline formatting, just get the text content
    ...
    def parse_table_to_text(table_element):
        """Convert table to simplified text representation."""
        rows = []
    ...
    def parse_block_element(element):
        ...
        # Tables - convert to simplified text
        elif element.name == 'table':
            table_text = parse_table_to_text(element)
            if table_text:
                return {
                    'type': 'paragraph',
                    'children': [{'type': 'text', 'value': table_text}]
                }
            return None
            
        # Special handling for hr (horizontal rule) and br (line break)
        ...
        # Paragraphs and other block elements
            ...
            # Skip table internal elements if processed separately
            if element.name in ['thead', 'tbody', 'tfoot', 'tr', 'td', 'th']:
                return None
                
            # Check if this block contains other blocks
            nested_blocks = element.find_all(list(BLOCK_TAGS | HEADING_TAGS | {'ul', 'ol'}), recursive=False)
            
            if nested_blocks:
                # This block contains other blocks, process them recursively
                results = []
                for child in element.children:
                    if isinstance(child, Tag) and child.name in (BLOCK_TAGS | HEADING_TAGS | {'ul', 'ol'}):
        ...
        for child in parent.children:
            if isinstance(child, NavigableString):
                ...
                if child.name in SKIP_TAGS:
                    ...
                    continue
                elif child.name in (BLOCK_TAGS | HEADING_TAGS | {'ul', 'ol'}):
